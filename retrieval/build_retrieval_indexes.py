...
DEFAULT_EMBEDDING_REPO = "Loctran123/vietnamese-evidence-corpus-embeddings-e5-large"
DEFAULT_OUTPUT_REPO = "Loctran123/vietnamese-evidence-retrieval-indexes"
...
def build_faiss_and_metadata(
    ...
    try:
        for shard_name in tqdm(shard_names, desc="FAISS + metadata", unit="shard"):
            ...
            duplicate_ids = seen_chunk_ids.intersection(chunk_ids)
            if duplicate_ids:
                raise RuntimeError(f"Duplicate chunk_id detected: {next(iter(duplicate_ids))}")
            seen_chunk_ids.update(chunk_ids)

            vectors = embedding_values(table)
            index.add(vectors)

            metadata_names = [name for name in table.column_names if name != "embedding"]
            metadata = table.select(metadata_names)
            row_ids = pa.array(range(row_offset, row_offset + len(metadata)), type=pa.int64())
    ...
    if index.ntotal != row_offset:
        raise RuntimeError(f"FAISS rows ({index.ntotal}) != metadata rows ({row_offset}).")
