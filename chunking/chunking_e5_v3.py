...
class Tokenizer(Protocol):
    """Phần giao diện tokenizer mà logic chunking sử dụng."""
...
BOILERPLATE_LINE_PATTERNS = [
    ...
    for pattern in [
        r"^xem\s+th(?:e|\u00eam)\s*:?.*$",
        r"^tin\s+kh(?:a|\u00e1)c\s*$",
        r"^tin\s+li(?:e|\u00ea)n\s+quan\s*$",
        r"^b(?:a|\u00e0)i\s+vi(?:e|\u1ebf)t\s+li(?:e|\u00ea)n\s+quan\s*$",
        r"^c(?:u|\u00f9)ng\s+chuy(?:e|\u00ea)n\s+m(?:u|\u1ee5)c\s*$",
        r"^theo\s+d(?:o|\u00f5)i\s+vnexpress.*$",
        r".*vnexpress\s+tr(?:e|\u00ea)n\s+google\s+news.*",
        r".*ch(?:o|\u1ecdn)\s+vnexpress.*google.*",
        r"^menu\s*$",
        r"^\s*(share|chia\s+s(?:e|\u1ebb)|comment|b(?:i|\u00ec)nh\s+lu(?:a|\u1ead)n)\s*$",
    ]
]
...
def chunk_document(
    ...
    """Chia trường ``text`` của một tài liệu và giữ metadata nguồn."""
    ...
    body_budget = max(1, chunk_size - len(prefix_tokens) - special_tokens)
    ...
    ranges = sentence_aware_token_ranges(
        len(token_ids), sentence_spans, body_budget, overlap, min_chunk_size
    )
    ...
    for chunk_index, (start, end) in enumerate(ranges):
        ...
        yield {
            **source_fields,
            ...
            "content_hash": hashlib.sha256(normalize_for_dedup(f"passage: {title.strip()}\n\n{chunk_text}").encode("utf-8")).hexdigest(),
            "text": chunk_text,
        }
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Chia trường text của corpus JSON theo token multilingual-E5."
    )
    parser.description = "Split corpus text into multilingual-E5 token chunks."
    ...
    parser.add_argument(
        ...
        type=int,
        default=512,
        help="Maximum multilingual-E5 tokens per chunk including prefix (default: 512)",
    )
    parser.add_argument(
        "--overlap",
        type=int,
