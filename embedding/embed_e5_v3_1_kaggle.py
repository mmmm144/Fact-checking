...
DEFAULT_SOURCE_REPO = os.getenv(
    "HF_CHUNK_REPO", "aiMy144/vietnamese-evidence-corpus-chunked-e5-v3-1"
)
DEFAULT_OUTPUT_REPO = os.getenv(
    "HF_EMBEDDING_REPO", "aiMy144/vietnamese-evidence-corpus-embeddings-e5-large-v3-1"
)
...
def select_row(row: dict[str, Any], text_column: str) -> dict[str, Any]:
    ...
    if text_column != "text":
        selected["text"] = text
    if "provenance" in row and "provenance" not in selected:
...
def provenance_entry(row: dict[str, Any]) -> dict[str, Any]:
    return {
...
def deduplicate_rows(rows: Iterable[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        content_hash = str(row.get("content_hash") or normalized_hash(text))
        entry = provenance_entry(row)

        if content_hash in hash_to_index:
            existing = deduped[hash_to_index[content_hash]]
        ...
        selected = dict(row)
        selected["content_hash"] = content_hash
        selected["provenance"] = [entry]
        hash_to_index[content_hash] = len(deduped)
...
def write_shard(
    ...
    for row, text in zip(rows, texts):
        ...
        entry = provenance_entry(row)
        if content_hash in hash_to_index:
            existing = dedup_rows[hash_to_index[content_hash]]
            provenance = list(existing.get("provenance") or [])
            if entry not in provenance:
                provenance.append(entry)
            existing["provenance"] = provenance
        ...
        selected = select_row(row, args.text_column)
        selected["content_hash"] = content_hash
        selected["provenance"] = row.get("provenance") or [entry]
        dedup_rows.append(selected)
        dedup_texts.append(text)
        hash_to_index[content_hash] = len(dedup_rows) - 1
    ...
    if embeddings.shape != (len(dedup_rows), 1024):
    ...
    column_names = [name for name in DEFAULT_COLUMNS if name in dedup_rows[0]]
    arrays: dict[str, pa.Array] = {
        name: pa.array([row.get(name) for row in dedup_rows]) for name in column_names
    }
    ...
    table = pa.table(arrays).replace_schema_metadata(
        {
            ...
            b"embedding_prefix": b"passage: {title}\\n\\n",
            b"normalized": b"true",
        }
    )
    ...
    pq.write_table(
        ...
        compression="zstd",
        compression_level=6,
        row_group_size=min(len(dedup_rows), 1_000),
    )
    temporary_path.replace(shard_path)
    return shard_path, len(dedup_rows)
...
- sentence-similarity
- feature-extraction
pretty_name: Vietnamese Evidence Corpus Embeddings (multilingual-e5-large v3)
---
...
inner-product retrieval. The source chunk boundaries and provenance columns are
kept for auditing, context assembly, and source tracing.
"""
...
def main() -> None:
    ...
    if not args.no_upload:
        ...
        api, _, _, complete = prepare_output_repo(args, token, config)
        if complete:
    ...
    print(
        f"Deduplicated {processed_now:,} source chunks to {len(dedup_rows):,} unique embedding rows."
    )

    embedded_now = 0
    for start in range(0, len(dedup_rows), args.rows_per_shard):
        shard_rows = dedup_rows[start : start + args.rows_per_shard]
        shard_path, embedded_rows = write_shard(shard_rows, model, args, shard_index)
        embedded_now += embedded_rows
        print(f"Wrote {shard_path} ({embedded_rows:,} deduplicated rows from {len(shard_rows):,} chunks)")
        if api is not None and token is not None:
